...
class HSFileUpload extends HSBasePlugin<IFileUploadOptions> implements IFileUpload {
  ...
  private reloadButtonClick(evt: MouseEvent, file: DropzoneFile) {
    ...
    tempFileInput.addEventListener('change', this.onTempFileInputChangeListener.find(el => el.el === tempFileInput).fn)
  }
  ...
  private initDropzone() {
    ...
    this.dropzone.on('uploadprogress', (file: DropzoneFile, progress: number) => this.onUploadProgress(file, progress))
  ...
  public destroy() {
    ...
    window.$hsFileUploadCollection = window.$hsFileUploadCollection.filter(({ element }) => element.el !== this.el)
  }
  ...
  private onAddFile(file: DropzoneFile) {
    ...
    if (reloadButton) {
      ...
      reloadButton.addEventListener('click', this.onReloadButtonClickListener.find(el => el.el === reloadButton).fn)
    }
  ...
  static autoInit() {
    ...
    document.querySelectorAll('[data-file-upload]:not(.--prevent-on-load-init)').forEach((el: HTMLElement) => {
      if (!window.$hsFileUploadCollection.find(elC => (elC?.element?.el as HTMLElement) === el)) new HSFileUpload(el)
    })
  }
}
