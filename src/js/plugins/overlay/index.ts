...
import { afterTransition, dispatch, getClassProperty, isDirectChild, stringToBoolean } from '../../utils'
...
class HSOverlay extends HSBasePlugin<{}> implements IOverlay {
  ...
  constructor(el: HTMLElement, options?: IOverlayOptions, events?: {}) {
    ...
    this.initiallyOpened = document.body.classList.contains('overlay-body-open')
  ...
  private elementMinifierClick() {
    const payloadFn = () => {
      ...
      dispatch('toggleMinifierClicked.overlay', this.el, payload)
    }
  ...
  private buildToggleMinifierButtons() {
    this.toggleMinifierButtons.forEach(el => {
      ...
      el.addEventListener(
        'click',
        this.onElementMinifierClickListener.find(minifierButton => minifierButton.el === el).fn
      )
    })
  }
  ...
  public updateToggles() {
    const found = Array.from(document.querySelectorAll<HTMLElement>(`[data-overlay="#${this.el.id}"]`))
    ...
    this.toggleButtons = this.toggleButtons.filter(btn => {
      ...
      const listener = this.onElementClickListener?.find(lst => lst.el === btn)
    ...
    this.accessibilityComponent = window.HSAccessibilityObserver.registerComponent(
      this.el,
      {
        ...
        onTab: () => {
          ...
          const currentIndex = focusedElement ? focusableElements.indexOf(focusedElement as HTMLElement) : -1
