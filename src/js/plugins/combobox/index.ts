...
class HSComboBox extends HSBasePlugin<IComboBoxOptions> implements IComboBox {
  ...
  private isItemExists(obj: never): boolean {
    return this.items.some((el: HTMLElement) => {
      const groupField = el.getAttribute('data-combo-box-output-item-group-field') ?? null
      const params = JSON.parse(el.getAttribute('data-combo-box-output-item')) ?? null
      let group = null
        ...
        const equality =
          params?.group?.name && group
            ? group === params.group.name &&
              elI.getAttribute('data-combo-box-search-text') ===
                obj[elI.getAttribute('data-combo-box-output-item-field')]
            : elI.getAttribute('data-combo-box-search-text') ===
              obj[elI.getAttribute('data-combo-box-output-item-field')]
  ...
  private jsonItemsRender(items: any) {
    items.forEach((item: never, index: number) => {
      ...
      newItem.querySelectorAll('[data-combo-box-search-text]').forEach(el => {
        el.setAttribute('data-combo-box-search-text', item[el.getAttribute('data-combo-box-output-item-field')] ?? '')
      })
