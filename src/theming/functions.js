...
const colorIsInvalid = input => {
  console.error(
    `├─ ${pc.red('⚠︎')} ${pc.bgRed(' Error ')} Invalid color ${pc.red(input)} in ${pc.green('tailwind.config.js')}`
  )
}
...
module.exports = {
  ...
  convertColorFormat: function (input) {
    ...
    for (const [rule, value] of Object.entries(input)) {
      ...
      //Shadow
      if (!Object.hasOwn(input, 'base-shadow')) {
        resultObj['--bs'] = this.generateDarkenColorFrom(input['base-100'], 0.8)
      }
  ...
  injectThemes: function (addBase, config, themes) {
    ...
    // add default themes
    const themeRoot = config('flyonui.themeRoot') ?? ':root'
    for (const [theme, value] of Object.entries(themes)) {
      includedThemesObj[theme] = this.convertColorFormat(value)
    }

    // add custom themes
    if (Array.isArray(config('flyonui.themes'))) {
      for (const item of config('flyonui.themes')) {
    ...
    let themeOrder = []
    if (Array.isArray(config('flyonui.themes'))) {
      for (const theme of config('flyonui.themes')) {
    ...
    } else if (config('flyonui.themes') === true) {
    ...
    themeOrder.forEach((themeName, index) => {
      if (index === 0) {
        ...
        // auto dark
        if (config('flyonui.darkTheme')) {
          if (themeOrder[0] !== config('flyonui.darkTheme') && themeOrder.includes(config('flyonui.darkTheme'))) {
            themesToInject['@media (prefers-color-scheme: dark)'] = {
              [themeRoot]: includedThemesObj[`${config('flyonui.darkTheme')}`]
            }
          }
        } else if (config('flyonui.darkTheme') === false) {
